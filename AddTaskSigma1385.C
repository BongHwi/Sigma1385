AliAnalysisTaskSigma1385* AddTaskSigma1385(const char *taskname = "Sigma1385"
                                     ...
                                     , const char* suffix = "MB")
{
    ...
    if(foption.Contains("MC"))
        IsMC = kTRUE;
    AliAnalysisTaskSigma1385 *taskSigma1385 = new AliAnalysisTaskSigma1385(Form("%s%s", taskname,suffix), IsMC);
    taskSigma1385->fEventCuts.fCentralityFramework = 1;
    std::cout << "AddTaskSigma1385:: Option: " << option << std::endl;
    if(foption.Contains("MC")){
        std::cout << "AliAnalysisTaskSigma1385:: MC mode " << std::endl;
    ...
    AliAnalysisDataContainer *coutputSigma1385 = mgr->CreateContainer(Form("%s%s", taskname, suffix), TList::Class(), AliAnalysisManager::kOutputContainer, "AnalysisResults.root");
    mgr->ConnectOutput(taskSigma1385, 1, coutputSigma1385);

    AliAnalysisDataContainer *coutputSigma1385Tuple = mgr->CreateContainer("Sigma1385",TNtupleD::Class(),AliAnalysisManager::kOutputContainer,"AnalysisResults.root");
