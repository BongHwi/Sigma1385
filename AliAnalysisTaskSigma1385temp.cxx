...
ClassImp(AliAnalysisTaskSigma1385temp) // classimp: necessary for root
...
AliAnalysisTaskSigma1385temp::AliAnalysisTaskSigma1385temp(const char* name, Bool_t MCcase) : 
    ...
    fNtupleSigma1385(0)
{
    ...
    DefineOutput(2, TNtupleD::Class());  // you can add more output objects by calling DefineOutput(2, classname::Class())
}
//_____________________________________________________________________________
AliAnalysisTaskSigma1385temp::~AliAnalysisTaskSigma1385temp(){}
...
//_____________________________________________________________________________
void AliAnalysisTaskSigma1385temp::UserCreateOutputObjects()
{
    ...
    auto binType = AxisStr("Type", {"SigmaStarP", "SigmaStarN", "AntiSigmaStarP", "AntiSigmaStarN",
                                    "SigmaStarP_mix", "SigmaStarN_mix", "AntiSigmaStarP_mix", "AntiSigmaStarN_mix",
                                    "SigmaStarP_gen", "SigmaStarN_gen", "AntiSigmaStarP_gen", "AntiSigmaStarN_gen",
                                    "SigmaStarP_rec", "SigmaStarN_rec", "AntiSigmaStarP_rec", "AntiSigmaStarN_rec"});
    
    std::vector<double> centaxisbin = {
            0, 1, 5, 10, 15, 20, 30, 40, 50, 60, 70, 80, 90, 100};  // can be use from pp to PbPb
    ...
    fNtupleSigma1385 = new TNtupleD("fNtupleSigma1385", "Sigma1385", "PIDSigmaStarPion:DCASigmaStarPionToPrimVertexZ:DCASigmaStarPionToPrimVertexR:EtaSigmaStarPion:PhiSigmaStarPion:PIDV0pTrackProton:PIDV0pTrackPion:PIDV0nTrackProton:PIDV0pTrackPion:DCAV0Daughters:DCAV0ToPrimVertex:CosPointingAngleV0:V0Mass:EtaV0:PhiV0:MCflag");

    PostData(1, fHistos->GetListOfHistograms());
    PostData(2, fNtupleSigma1385);
}
//_____________________________________________________________________________
void AliAnalysisTaskSigma1385temp::UserExec(Option_t *)
{
...
//_____________________________________________________________________________
Bool_t AliAnalysisTaskSigma1385temp::GoodTracksSelection() {
    ...
    for (UInt_t it = 0; it < nTracks; it++) {
        ...
        Double_t pionDCA_r = sqrt(pow(track->GetX(),2)+pow(track->GetY(),2));
...
Bool_t AliAnalysisTaskSigma1385temp::GoodV0Selection() {
    ...
    if (fEvt->IsA() == AliESDEvent::Class()) {  // ESD case
        for (UInt_t it = 0; it < nV0; it++) {
            ...
            AliESDtrack* pTrackV0 =
                ((AliESDEvent*)fEvt)
                    ->GetTrack(TMath::Abs(v0ESD->GetPindex()));
            AliESDtrack* nTrackV0 =
                ((AliESDEvent*)fEvt)
                    ->GetTrack(TMath::Abs(v0ESD->GetNindex()));
            if ( TMath::Abs( ((pTrackV0->GetSign()) - (nTrackV0->GetSign())) ) < 0.1)
                AcceptedV0 = kFALSE;

            // PID cuts
            Double_t fTPCNSigProton =
                GetTPCnSigma(pTrackV0, AliPID::kProton);
            Double_t fTPCNSigAntiProton =
                GetTPCnSigma(nTrackV0, AliPID::kProton);
            Double_t fTPCNSigPion =
                GetTPCnSigma(nTrackV0, AliPID::kPion);
            Double_t fTPCNSigAntiPion =
                GetTPCnSigma(pTrackV0, AliPID::kPion);
            
            Bool_t fPIDLambda = (abs(fTPCNSigProton) < fTPCNsigLambdaProtonCut) 
                                && (abs(fTPCNSigPion) < fTPCNsigLambdaPionCut);
            Bool_t fPIDAntiLambda = (abs(fTPCNSigAntiProton) < fTPCNsigLambdaProtonCut) 
            ...
            // DCA to PV
            Double_t fDCADistLambda_PV =
                fabs(v0ESD->GetD(PVx, PVy, PVz));
        ...
        for (UInt_t it = 0; it < nV0; it++) {
            ...
            if (TMath::Abs(v0AOD->GetPosID()) ==
                TMath::Abs(v0AOD->GetNegID()))
                continue;
    ...
    for (UInt_t i = 0; i < nV0; i++) {
        if (fEvt->IsA() == AliESDEvent::Class()){
            ...
            fPIDLambda = (abs(fTPCNSigProton) < fTPCNsigLambdaProtonCut) 
                            && (abs(fTPCNSigPion) < fTPCNsigLambdaPionCut);
            fPIDAntiLambda = (abs(fTPCNSigAntiProton) < fTPCNsigLambdaProtonCut) 
                            && (abs(fTPCNSigAntiPion) < fTPCNsigLambdaPionCut);
            if(fPIDLambda)
                v0ESD->ChangeMassHypothesis(kLambda0);
            if(fPIDAntiLambda)
            ...
            temp1.SetXYZM(v0ESD->Px(), v0ESD->Py(),
                        v0ESD->Pz(), v0ESD->GetEffMass());
        }
        else{
            ...
            if(fPIDLambda)
                temp1.SetXYZM(v0AOD->MomV0X(), v0AOD->MomV0Y(),
                            v0AOD->MomV0Z(), v0AOD->MassLambda());
            if(fPIDAntiLambda)
                temp1.SetXYZM(v0AOD->MomV0X(), v0AOD->MomV0Y(),
                            v0AOD->MomV0Z(), v0AOD->MassAntiLambda());
        }
        ...
        for (UInt_t j = 0; j < nTracks; j++) {
            ...
            if (IsMC && IsTrueSigmaStar(goodv0indices[i],goodtrackindices[j])) {
...
void AliAnalysisTaskSigma1385temp::FillNtuples() {
    ...
    for (UInt_t i = 0; i < nV0; i++) {
        if (fEvt->IsA() == AliESDEvent::Class()){
            ...
            tmp[11] = v0ESD->GetV0CosineOfPointingAngle(PVx, PVy, PVz);//CosPointingAngleV0
        ...
        for (UInt_t j = 0; j < nTracks; j++) {
            ...
            tmp[2] = sqrt(pow(track1->GetX(),2)+pow(track1->GetY(),2)); //DCASigmaStarPionToPrimVertexR
...
void AliAnalysisTaskSigma1385temp::FillMCinput(AliMCEvent* fMCEvent) {
    auto sign = kAllType;
    if (fEvt->IsA() == AliESDEvent::Class()){
...
Bool_t AliAnalysisTaskSigma1385temp::IsTrueSigmaStar(UInt_t v0Index, UInt_t pionIndex){
    ...
    if (fEvt->IsA() == AliESDEvent::Class()){
        ...
        TParticle* MCLamD1 = (TParticle*)fMCEvent->GetTrack(abs(pTrackV0->GetLabel()))->Particle();
        TParticle* MCLamD2 = (TParticle*)fMCEvent->GetTrack(abs(nTrackV0->GetLabel()))->Particle();
        TParticle* MCLam;
        TParticle* MCPi;
        ...
        if ((abs(MCLamD1->GetPdgCode()) == kProtonCode &&
            ...
            (abs(MCLamD1->GetPdgCode()) == kPionCode &&
             abs(MCLamD2->GetPdgCode()) ==
                 kProtonCode)) {
            if (MCLamD1->GetMother(0) ==
                MCLamD2->GetMother(0)) {
                MCLam = (TParticle*)fMCEvent
                           ->GetTrack(abs(MCLamD1->GetMother(0)))
                           ->Particle();
                if(abs(MCLam->GetPdgCode()) ==
                        kLambdaCode){
                    MCPi = (TParticle*)fMCEvent
                                ->GetTrack(abs(track1->GetLabel()))
                                ->Particle();
                    if(abs(MCPi->GetPdgCode()) == kPionCode){
                        if (MCPi->GetMother(0) ==
                            ...
                            MCSigmaStar = (TParticle*)fMCEvent
                                            ...
                                            ->Particle();
                            if((abs(MCSigmaStar->GetPdgCode()) == kSigmaStarPCode)
                                || (abs(MCSigmaStar->GetPdgCode()) == kSigmaStarNCode)){
                                if (IsPrimaryMC) {
        ...
        AliAODMCParticle* MCLamD1 = (AliAODMCParticle*)fMCArray->At(abs(pTrackV0->GetLabel()));
        AliAODMCParticle* MCLamD2 = (AliAODMCParticle*)fMCArray->At(abs(nTrackV0->GetLabel()));
        ...
        if ((abs(MCLamD1->GetPdgCode()) == kProtonCode &&
            ...
            if (MCLamD1->GetMother() ==
                MCLamD2->GetMother()) {
                MCLam = (AliAODMCParticle*)fMCArray->At(
                        abs(MCLamD1->GetMother()));
                if(abs(MCLam->GetPdgCode()) ==
                        kLambdaCode){
                    MCPi = (AliAODMCParticle*)fMCArray->At(abs(track1->GetLabel()));
                    if(abs(MCPi->GetPdgCode()) == kPionCode){
                        if (MCLam->GetMother() ==
                            MCPi->GetMother()) {
...
THnSparse* AliAnalysisTaskSigma1385temp::CreateTHnSparse(TString name,
                                                      TString title,
                                                      Int_t ndim,
...
Long64_t AliAnalysisTaskSigma1385temp::FillTHnSparse(TString name,
...
Long64_t AliAnalysisTaskSigma1385temp::FillTHnSparse(THnSparse* h,
                                                  std::vector<Double_t> x,
                                                  Double_t w) {
    // From AliPhysics/PWGUD/DIFFRACTIVE/Resonance/AliAnalysisTaskf0f2.cxx
    // Original author: Beomkyu Kim
...
TAxis AliAnalysisTaskSigma1385temp::AxisFix(TString name,
                                         int nbin,
                                         Double_t xmin,
                                         Double_t xmax) {
    // From AliPhysics/PWGUD/DIFFRACTIVE/Resonance/AliAnalysisTaskf0f2.cxx
...
TAxis AliAnalysisTaskSigma1385temp::AxisStr(TString name,
...
TAxis AliAnalysisTaskSigma1385temp::AxisVar(TString name,
...
double AliAnalysisTaskSigma1385temp::GetTPCnSigma(AliVTrack* track,
